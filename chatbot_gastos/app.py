...
def get_user_data(user_phone):
    ...
    user_df = df[df['Identificador'] == user_phone].copy()
...
def generate_summary_report(user_phone, start_date, end_date, title):
    ...
    try:
        ...
        for category, total in expenses_by_category.sort_values(ascending=False).items():
            report_lines.append(f"• {category.capitalize()}: *R$ {total:,.2f}*".replace(',', '.'))
        
        report_lines.append("\n-----------------------------------")
        report_lines.append(f"*Total Gasto no Período: R$ {total_spent:,.2f}*".replace(',', '.'))
...
def generate_detailed_statement(user_phone, start_date, end_date, title):
    ...
    try:
        ...
        for index, row in statement_df.iterrows():
            ...
            value = row['Valor']
            report_lines.append(f"• {date_str} - {category.capitalize()}: *R$ {value:,.2f}*".replace(',', '.'))
        
        total_spent = statement_df['Valor'].sum()
        report_lines.append("\n-----------------------------------")
        report_lines.append(f"*Total do Período: R$ {total_spent:,.2f}*".replace(',', '.'))
    ...
    try:
        ...
        if data.get("event") == "messages.upsert" and data.get("data", {}).get("key", {}).get("fromMe") is False:
            ...
            message_body = (message_data.get("message", {}).get("conversation") or \
                           ...
                           "").strip().lower() # Converte para minúsculas e remove espaços
            ...
            current_month = ["este mês", "este mes", "este", "atual", "deste mês", "deste mês", "deste", "mês atual", "mes atual"]
            ...
            if user_phone in user_states:
                ...
                # --- FLUXO DE ESCOLHA DO MÊS ---
                    ...
                    if message_body in current_month:
                        ...
                        start_date = end_of_last_month.replace(day=1, hour=0, minute=0)
                    ...
                    try:
                        ...
                        if 0 <= rating <= 5:
                            ...
                            if rating <= 3:
                                ...
                                user_states[user_phone] = 'awaiting_feedback' # Muda o estado para aguardar o feedback
                            else:
            ...
            if  message_body in suggestion:
                ...
                user_states[user_phone] = 'awaiting_rating'
                ...
                if is_new_user(user_phone):
                    ...
                    # Registra o primeiro contato na planilha para que ele não seja mais 'novo'
                    sheet.append_row([user_phone, datetime.now().strftime('%Y-%m-%d'), 0, 'PRIMEIRO_CONTATO', datetime.now().strftime('%Y-%m-%d')])
                else:
